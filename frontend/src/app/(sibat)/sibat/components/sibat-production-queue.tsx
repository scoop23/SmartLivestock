...
interface SibatProductionQueueProps {
  ...
  onProdTypeFilterChange: (type: string) => void;
}
...
const getCommodityEmoji = (title: string = "") => {
  ...
  if (t.includes("wool")) return "🧶";
  return "🌾";
};

export default function SibatProductionQueue({
  submissions,
  isLoading,
  onReview,
  statusFilter,
  ...
  const filteredSubmissions = useMemo(() => {
    ...
    return prodSubmissions.filter((item) => {
      const matchesStatus =
        statusFilter === "all" ||
  ...
  }, [prodSubmissions, statusFilter, prodTypeFilter, searchQuery]);

  const hasActiveFilters =
    searchQuery.trim() !== "" || statusFilter !== "all" || prodTypeFilter !== "ALL";

  return (
    <div className="space-y-4">
      {/* Friendly Header */}
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2 bg-sky-50/60 border border-sky-100 p-4 rounded-3xl">
        <div className="flex items-center gap-3">
          <div className="size-10 rounded-2xl bg-sky-500 text-white flex items-center justify-center font-black shadow-xs">
        ...
        {counts.pending > 0 && (
          <Badge className="bg-sky-600 text-white font-extrabold text-xs px-3 py-1 rounded-full shadow-2xs self-start sm:self-center">
            {counts.pending} {counts.pending === 1 ? "log to calibrate" : "logs to calibrate"}
          </Badge>
        )}
      </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-200/80 shadow-2xs p-3.5 space-y-3">
        ...
        <div className="flex flex-col md:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <Input
              placeholder="Search by farmer name, barangay, livestock type, yield..."
              value={searchQuery}
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-3xl border border-dashed border-slate-200 p-12 text-center space-y-3 shadow-2xs">
          <div className="size-14 rounded-3xl bg-sky-50 text-sky-600 flex items-center justify-center mx-auto text-2xl font-black">
            🥛
          </div>
          <h3 className="text-base font-black text-slate-800">
            {hasActiveFilters ? "No matching production logs" : "All Caught Up on Yield Logs!"}
          </h3>
          <p className="text-xs text-slate-500 max-w-md mx-auto font-medium">
            {hasActiveFilters
              ? "No yield logs matched your search or filters. Try adjusting them!"
              : "No farmer production yields are waiting for calibration right now."}
          </p>
        </div>
      ) : (
        <div className="space-y-3">
                ...
                <CardContent className="p-4 sm:p-5">
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    ...
                    <div className="flex items-start gap-3.5 min-w-0 flex-1">
                      <div className="size-12 rounded-2xl shrink-0 flex items-center justify-center font-black text-2xl bg-sky-50 text-sky-700 border border-sky-200/80 shadow-2xs">
                        {emoji}
                      </div>

                      <div className="min-w-0 flex-1 space-y-1.5">
                        <div className="flex items-center gap-2 flex-wrap">
                          <Badge className="bg-sky-100 text-sky-800 border-sky-200 text-[10px] font-black uppercase rounded-md px-2 py-0.5">
                        ...
                        {item.reviewRemarks && (
                          <div className="text-[11px] bg-sky-50 text-sky-950 p-2.5 rounded-2xl border border-sky-200/80 mt-1">
                            <strong className="font-bold text-sky-900">✨ Your Calibration Note: </strong>
                            {item.reviewRemarks}
                          </div>
                        )}
                      </div>
