...
import { Users, CheckSquare, Map, TrendingUp, AlertTriangle, Sprout, FileText, Download, FileSpreadsheet, FileBarChart, Database } from 'lucide-react';
      ...
      <main className="flex-1 overflow-auto">
        ...
        <div className="bg-white border-b border-gray-200 p-4 md:p-6">
          <div className="max-w-7xl mx-auto">
            <h1 className="text-2xl mb-1">LGU/MAO Dashboard</h1>
        ...
        <div className="p-4 md:p-6 max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
              <h3 className="mb-4">Cattle Distribution by Barangay</h3>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={cattleDistribution}>
                  ...
                  <YAxis />
                  <Tooltip />
                  <Bar dataKey="cattle" fill="#2D5A27" />
                </BarChart>
              </ResponsiveContainer>
            </div>

            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
              <h3 className="mb-4">Livestock Type Distribution</h3>
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  <Pie
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 mb-6">
            <h3 className="mb-4">Monthly Production Trends</h3>
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={monthlyProduction}>
                ...
                <Tooltip />
                <Legend />
                <Line yAxisId="left" type="monotone" dataKey="milk" stroke="#2D5A27" name="Milk (L)" strokeWidth={2} />
                <Line yAxisId="right" type="monotone" dataKey="meat" stroke="#D32F2F" name="Meat (kg)" strokeWidth={2} />
              </LineChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 mb-6">
            ...
            <p className="text-sm text-gray-600 mb-6">
              Export comprehensive reports for Department of Agriculture submissions
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {[
                ...
                <div key={title} className="border border-gray-200 rounded-lg p-4 hover:border-[#2D5A27] transition-colors">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { path: '/data-overview', icon: Database, label: 'System Data Overview', desc: 'View all inputted data' },
              { path: '/user-management', icon: Users, label: 'User Management', desc: 'Manage farmer accounts' },
              { path: '/data-validation', icon: CheckSquare, label: 'Data Validation', desc: 'Review pending records' },
              { path: '/gis-map', icon: Map, label: 'GIS Map', desc: 'View cattle distribution' },
              { path: '/analytics', icon: TrendingUp, label: 'Advanced Analytics', desc: 'AI predictions & insights' },
            ].map(({ path, icon: Icon, label, desc }) => (
              <button
                key={path}
                onClick={() => router.push(path)}
                className="bg-white p-6 rounded-lg shadow-sm border-2 border-gray-200 hover:border-[#2D5A27] transition-colors text-left"
              >
                <Icon className="w-8 h-8 text-[#2D5A27] mb-3" />
