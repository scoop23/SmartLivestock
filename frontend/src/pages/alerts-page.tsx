    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-gray-50">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-4 py-3 z-30">
        <div className="flex items-center justify-around max-w-lg mx-auto">
          <button onClick={() => router.push('/farmer')} className="flex flex-col items-center gap-1 text-gray-600">
            <span className="text-xs">Home</span>
          </button>
          <button onClick={() => router.push('/livestock-inventory')} className="flex flex-col items-center gap-1 text-gray-600">
            <span className="text-xs">Inventory</span>
          </button>
          <button onClick={() => router.push('/production-logger')} className="flex flex-col items-center gap-1 text-gray-600">
            <span className="text-xs">Logger</span>
          </button>
          <button className="flex flex-col items-center gap-1 text-[#2D5A27] relative">
            <span className="text-xs">Alerts</span>
            {alerts.filter(a => !a.read).length > 0 && (
              <span className="absolute -top-1 right-4 w-2 h-2 bg-red-600 rounded-full"></span>
            )}
          </button>
        </div>
      </nav>
